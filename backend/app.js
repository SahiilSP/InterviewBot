...
const resultSet = [
    {
        q: 'Explain the difference between TCP and UDP protocols. Which protocol is more suitable for streaming media applications and why?',
        text: "TCP (Transmission Control Protocol) is a connection-oriented protocol that ensures reliable, ordered delivery of packets, while UDP (User Datagram Protocol) is a connectionless protocol that does not guarantee delivery or order of packets. UDP is more suitable for streaming media applications because it has lower overhead and can handle loss of some packets without affecting the overall stream quality.",
        issim: "false",
        qwpm: 107,
        simscore: [0.72455,0.68034],
        sentiment: {score:0.67,
        sentiment:"POSITIVE"},
        grammar: [],
    },
    {
        q: 'Describe the Naive Bayes algorithm in machine learning. How does it handle categorical and continuous features?',
        text: "The Naive Bayes algorithm in machine learning is a probabilistic classifier based on Bayes' theorem. It assumes that features are independent of each other, hence naive. It handles categorical features by calculating probabilities directly from the frequency counts of each category, and it handles continuous features by assuming a Gaussian (normal) distribution and calculating the probability density function.",
        issim: "false",
        qwpm: 90,
        simscore: [0.84455,0.68034],
...
async function gettextsimi(idx, recans) {
    ...
    const resp = await fetch("https://api.app.labelf.ai/v2/similarity", {
        ...
        body: JSON.stringify({
            top_n: 2,
            base_texts: { a1: QAset[idx].answer1, a2: QAset[idx].answer2 },
            compare_to_texts: { comp: recans },
        }),
    });
    const data = await resp.json()
    console.log(data);
    obj.simscore.push(data.comp[0].similarity);
    obj.simscore.push(data.comp[1].similarity);
}
// gettextsimi()
...
app.post('/getR', (req, res) => {
    ...
    const wpm = Math.floor(string1.length * 60 / (anstime / 1000));
    console.log(wpm, ": words per minute");
    obj.qwpm = wpm;
    obj.text = string1;
    obj.q = questions[aidx];
    console.log(obj);
    resultSet.push(obj);
    // resultSet.push({
    //     issim: areSimilar[0],
